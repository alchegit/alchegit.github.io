...
create table storyheaven_episodes (
  ...
  episode_status varchar2(30) default 'draft' not null check (episode_status in ('draft', 'moderation', 'published', 'archived')),
  review_decision varchar2(30) default 'none' not null check (review_decision in ('none', 'pending', 'approved', 'changes_requested', 'rejected')),
  review_note varchar2(1000 char),
...
create index idx_sh_episode_public
  on storyheaven_episodes(story_id, episode_status, episode_no);

create index idx_sh_episode_moderation
  on storyheaven_episodes(episode_status, submitted_at);
...
create table storyheaven_episode_revisions (
  ...
  revision_no number(6) not null,
  actor_user_id varchar2(80) not null references webtoon_profiles(user_id),
  revision_kind varchar2(20) default 'draft' not null check (revision_kind in ('draft', 'submit', 'review', 'publish')),
  title varchar2(120 char) not null,
  public_summary varchar2(1000 char),
  body_text clob,
  content_hash varchar2(64) not null,
...
create index idx_sh_episode_revision
  on storyheaven_episode_revisions(episode_id, revision_no desc);

create index idx_sh_ep_revision_hash
  on storyheaven_episode_revisions(content_hash, created_at desc);
...
create index idx_sh_ai_batch_story
  on storyheaven_ai_review_batches(story_id, created_at desc);

create index idx_sh_ai_batch_queue
  on storyheaven_ai_review_batches(batch_status, next_attempt_at, created_at);
...
create index idx_sh_ai_review_queue
  on storyheaven_ai_reviews(review_status, next_attempt_at, created_at);

create index idx_sh_ai_review_batch
  on storyheaven_ai_reviews(batch_id, review_status, created_at);

create index idx_sh_ai_review_lease
  on storyheaven_ai_reviews(lease_id, lease_expires_at);
...
create index idx_sh_reading_recent
  on storyheaven_reading_progress(user_id, updated_at desc);

create table storyheaven_episode_reactions (
  episode_id varchar2(36) not null references storyheaven_episodes(id),
  user_id varchar2(80) not null references webtoon_profiles(user_id),
  reaction_type varchar2(30) not null check (reaction_type in ('next_episode', 'character', 'world', 'tension')),
  created_at timestamp with time zone default systimestamp not null,
...
create table storyheaven_notifications (
  ...
  notification_type varchar2(30) not null check (notification_type in ('report_result', 'appeal_received', 'appeal_result', 'episode_review_result')),
  title varchar2(120 char) not null,
