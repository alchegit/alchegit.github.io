-- Give the preserved build-bible job a fresh retry budget after aligning the
-- worker contract with the API validator. Keep the prior validation code so
-- the repaired worker can give the next generation attempt targeted feedback.

whenever sqlerror exit failure rollback

declare
  c_recovery_id constant varchar2(80) := '20260809-bible-contract-v1';
  ...
  if v_applied = 0 then
    update storyheaven_serial_jobs
     ...
     where id = c_job_id
       ...
       and error_code = 'review_api_422_serial_world_dynamic_force_invalid';
    v_affected_jobs := sql%rowcount;

    if v_affected_jobs <> 1 then
      raise_application_error(-20052, 'unexpected_bible_recovery_job_count');
    end if;
