...
function stageInstruction(type, payload = {}) {
  ...
  if (type === "build_episode_card") {
    let developmentRule = hasStoryDevelopmentCore(payload)
    ...
    if (payload?.rewriteTarget?.continuityMemoryPlan) {
      developmentRule += " This is a published-episode replacement. Copy payload.rewriteTarget.continuityMemoryPlan exactly into continuityMemoryPlan. It is server-owned history: do not address it again, rename its keys, or create replacement promises and debts.";
    }
  ...
  if (type === "revise_episode_card") {
    return "Repair the supplied current episode card as a senior structural editor after prose rewrites failed. Preserve the episode number, human desire, delivered genre rewards, successful character choices, relationship movement, continuity-memory commitments, prologue disclosure boundary, and all valid canon references. Copy payload.currentCard.continuityMemoryPlan exactly into continuityMemoryPlan; it is server-owned published history and must not be renamed, replaced, resolved again, or regenerated. Change only the causal mechanism, rule eligibility, information order, scene blocking, or hook that the final editor proved unusable. For every power, contract, promise, debt, authority, clue, or physical solution, name an exact existing world rule or canon fact and put the observable eligibility evidence before the effect. An opponent, witness, enforcer, or nearby person is not automatically a contract party or power target. Do not add a new power exception, institution, secret, event, or premise. Return a complete replacement episode card; the server supersedes the prior card and writes a fresh draft from this one.";
  }
  if (type === "write_draft") {
