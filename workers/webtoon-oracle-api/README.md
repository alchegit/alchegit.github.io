...
Editorial policy v18 separates new concept divergence from editor selection and rejects any selection response that mutates the writer's four candidates. New drafts receive six context-isolated critic jobs before the final editorial decision; the API also rejects a final response that rewrites the critic packet. Approved development-v2 episodes update a bounded serial memory containing unresolved reader promises, emotional debts, successful scene assets, and recent rhythm history. After the prologue and first two main installments exist, the API records a pilot readiness assessment for later operator promotion controls. Apply `oracle/20260809-storyheaven-development-pipeline.sql` before deploying workers that claim the new job types.
