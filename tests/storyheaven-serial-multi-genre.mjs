...
try {
  for (const viewport of [
    ...
    await page.route(apiPattern, async (route) => {
      ...
      if (path === "/api/storyheaven/operator/serial-engine/schedules" && request.method() === "GET") {
        return json({ enabled: true, schedules: [runningSchedule], queue: {
          ...
          lastCompleted: { workLabel: "판타지 · 3화까지", elapsedSeconds: 754, completedJobs: 14, completedAt: "2026-07-31T04:00:00.000Z" },
    ...
    await page.waitForFunction(() => JSON.parse(localStorage.getItem("storyheaven.operator.serial-draft.v5") || "null")?.primaryGenres?.length === 3);
    ...
    assert.match(await liveProgress.textContent(), /64%/u, `${viewport.name} live overview exposes numeric progress`);
    assert.equal(await liveProgress.locator('[role="progressbar"]').getAttribute("aria-valuenow"), "64", `${viewport.name} live overview is accessible`);
    ...
    assert.match(await scheduleProgress.textContent(), /64%/u, `${viewport.name} schedule meter exposes numeric progress`);
    assert.equal(await page.locator(".queue-row").count(), 2, `${viewport.name} running and waiting queue are visible`);
    const runningProgress = page.locator(".queue-row.running .production-progress");
    assert.equal(await runningProgress.count(), 1, `${viewport.name} running work has a progress train`);
    assert.match(await runningProgress.locator(".production-progress-heading").textContent(), /현재 · 2화.*4 \/ 7단계 완료/u, `${viewport.name} current production stage is explicit`);
    assert.equal(await runningProgress.locator(".production-step.is-complete").count(), 4, `${viewport.name} completed stages are filled`);
    assert.equal(await runningProgress.locator(".production-step.is-current").textContent(), "052화진행 중", `${viewport.name} current stage is highlighted`);
    assert.equal(await runningProgress.locator(".production-step.is-upcoming").count(), 2, `${viewport.name} planned stages remain unfilled`);
    ...
    await page.locator("[data-queue-live]").getByRole("button", { name: "중단 지점부터 재개" }).click();
